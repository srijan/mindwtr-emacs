...
### Sync cycle
One round trip of the sync engine: parse every surface into a Candidate, check whether anything changed locally or remotely, PUT the Candidate, GET the server's merged AppData, Reconcile each surface to it, save, and commit the Shadow. A cycle runs asynchronously across network callbacks, so everything a later stage needs is carried on the cycle itself rather than in a local binding. Only one cycle may be in flight at a time: a trigger that arrives while one is running (a timer, a retry, a manual sync) stands down, and a cycle aborts before its PUT if the user edits the buffer while it waits on the network. No cycle starts while an org-capture into a synced file is live: automatic triggers stand down silently and a manual sync refuses, because the rebuild would strand the capture's position in the file. A retry stood down this way stays scheduled without counting as a failed attempt.
...
### Conflict
A local edit the server overrode: this client proposed a change to an entity in its Candidate, but the merged AppData the server returned carries a different value, because the server's copy was at a newer revision. The sync report lists each conflict with a field diff and who wrote the winning version, and offers to restore the local edit. Restoring is the right remedy when another device made the winning change, and the wrong one when an Integrity repair made it.

### Integrity repair
A pass the server runs on every write, after merging, that fixes references the merge left dangling or contradictory. It clears a container reference (area, project, section) that points at a deleted or missing entity, merges duplicate live areas by name and repoints their references, tombstones sections whose project was deleted, and enforces the one-container-per-task rule. Every entity it changes gets a new revision attributed to the repair itself rather than to a device, so the repair outranks the edit just proposed and shows up as a Conflict, distinguishable from a concurrent edit only by that attribution. A write whose own payload already carries the dangling reference is rejected outright; the silent repair happens only when a reference goes dangling during the merge with stored data.
