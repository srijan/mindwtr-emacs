...
writes the chosen name to the CATEGORY property; `:areaId' already round-trips,
so there is no sync-seam work.  Creating a new area is out of scope.

Refuses on a task that already sits under a project or section: the area
CATEGORY is stamped to `:areaId' unconditionally for every kind, while a task's
`:projectId' comes from outline nesting, so setting an area there would parse
the task with BOTH a project and an area -- the dual-container over-stamp that
