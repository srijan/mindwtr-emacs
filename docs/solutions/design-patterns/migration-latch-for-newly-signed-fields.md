...
**1. A persistent one-way latch in the shadow** (`mindwtr-shadow.el`). It mirrors the existing etag flag-file pattern already used by the shadow, so it is a familiar shape, not a new persistence mechanism: *(session history)*
...
**2. Pre-migration protection in the merge** (`mindwtr-sync--merge-content` gained a `protected-field` argument). While the latch is unset, an *empty* local value for the protected notes field keeps the shadow value instead of clearing it. A real edited value is still adopted — protection suppresses only an empty local value, never a genuine edit:
...
`mindwtr-sync-build-candidate` resolves the protected field per kind via the registry `mindwtr-model-notes-field` (project → `:supportNotes`, section → `:description`), but **only for non-task kinds and only while the latch is unset**:
...
**3. Latch only after a confirmed save** (`mindwtr-sync-once`). The candidate is built with protection on until migrated:
...
and after the full cycle reconciles the buffer (rendering notes for the first time) and the buffer is saved to disk, the latch is set — *guarded on the save having succeeded*:
...
(let ((save-failed (null (mindwtr-sync--save-buffer-quietly t))))
  ...
  ;; durably on disk. ... If the save failed, the .org on disk may still
  ;; hold the old note-less render; latching now would drop empty-notes
  ;; protection, and a later reload from that stale file would clear a
  ;; server note via LWW.
  (unless save-failed
    (condition-case err
...
**Why latch-after-confirmed-save and not optimistically (commit `c195585`).** The latch was originally set right after `mindwtr-reconcile-buffer`, *before* the buffer was saved. A second code-review round flagged this as a data-loss path: if the save fails (read-only FS, full disk), the latch records "migrated" while the on-disk `.org` still holds the old note-less render. A later reload from that stale file parses empty notes with protection now **off**, and clears the server note via LWW — the exact bug the latch was built to prevent, merely deferred by one cycle. *(session history)*

The ordering rule is therefore: **the latch must fire only after the migrated state is durably observable.** The thing the latch *attests to* is "the note is rendered in the on-disk buffer," and that fact is not true until the save confirms. Latching on intent rather than on confirmed durability re-exposes the clobber. The latch write is additionally `condition-case`-guarded because it runs post-PUT — the server has already committed, so a latch-write hiccup must not throw and fail the sync.
...
- `mindwtr-sync-once-save-failure-does-not-latch-notes-migration` — with `save-buffer` stubbed to error ("disk full"), the cycle returns `:save-failed t` and `mindwtr-shadow-notes-migrated-p` stays `nil`, so protection survives for the next sync.
